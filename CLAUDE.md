...
2. **The GitHub API tools (`create_or_update_file` / `push_files`) work but don't scale to full-size images, AND `create_or_update_file` replaces the ENTIRE file -- it is not a patch/diff tool.** Always fetch the current full content first and submit the complete file back with your change merged in, even for a one-line edit to a doc like this one -- a partial submission silently truncates everything else (this happened to this very file on 2026-09-04, caught and fixed within the same session). For images: base64-encoding a ~500KB JPG is ~650KB of text, and inlining 10+ of those in one exchange is slow, wasteful, and risks hitting practical per-message output limits. Fine for one or two small images or text-file edits; not for a batch of full-size images.
